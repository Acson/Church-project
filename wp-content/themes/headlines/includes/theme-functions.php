...
// Use Google font replacement
function woo_google_font() {
	echo '<link href="http://fonts.googleapis.com/css?family=Droid+Sans" rel="stylesheet" type="text/css">' . "\n";
	echo '<style type="text/css">h1, h2, h3, h4, h5, h6, .cufon  { font-family: "Droid Sans", sans-serif !important; }</style>';

} 
add_action('wp_head','woo_google_font');
...
if ( function_exists('wp_nav_menu') ) {
	...
	if ( get_option('woo_menu_desc') == "true" ) {
		...
		function description_in_nav_el($item_output, $item, $depth, $args) {
			return preg_replace('/(<a.*?>[^<]*?)</', '$1' . "<br /><span class=\"description\">{$item->post_content}</span><", $item_output);
		}
	}
